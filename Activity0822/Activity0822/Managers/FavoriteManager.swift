...
extension FavoriteManager {
    ...
    func getCollectedEvents(_ completion: @escaping (_ events: [Event]) -> Void) {
        guard let currentUser = Auth.auth().currentUser else {
            completion([])
            return
        }
...
extension FavoriteManager {
    ...
    func attendEvent(_ event: Event) {
        guard let currentUser = Auth.auth().currentUser else { return }

        let docRef = Firestore.firestore().collection("AttendedEvents").document("\(event.name)")
    ...
    func removeAttendedEvent(_ event: Event) {
        guard let currentUser = Auth.auth().currentUser else { return }
    ...
    func isEventAttended(_ event: Event, completion: @escaping (_ collected: Bool) -> Void) {
        guard let currentUser = Auth.auth().currentUser else {
            completion(false)
            return
        }
            ...
            if let document = document, document.exists {
                ...
                if let event = Event(dictionary: document.data()!) {
                    for (_, user) in event.users.enumerated() {
                        if user == "\(currentUser.uid)" {
                            isCollected = true
                            log.debug("Event \(event.name) is attended")
                        }
                    }
                }
                completion(isCollected)
                ...
                log.debug("Event \(event.name) is not attended")
                completion(false)
            }
        }
    }

    func getAttendedEvents(_ completion: @escaping (_ events: [Event]) -> Void) {
        guard let currentUser = Auth.auth().currentUser else {
            completion([])
            return
        }

        let collection = Firestore.firestore().collection("AttendedEvents")
